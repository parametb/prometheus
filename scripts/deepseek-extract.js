...
async function createSourceRecord({ companyId }) {
  const sourceTypeMap = {
    '10-K': 'Annual Report',
    '10-Q': 'Annual Report',
  ...
  const tagsMap = {
    '10-K': ['financials', 'strategy'],
    'Annual Report': ['financials', 'strategy'],
    'Earnings Call': ['earnings', 'guidance'],
    'Press Release': ['earnings'],
    '10-Q': ['financials', 'earnings'],
    'SEC Filing': ['financials'],
  };

  const sourceTitleProp = await getTitlePropertyName(DB.sources);
  const sourceTitle = `${TICKER} ${DOC_TYPE} ${YEAR}`;

  return createPage(DB.sources, cleanProps({
    [sourceTitleProp]: n.title(sourceTitle),
...
async function main() {
  ...
  const sourcePage = await createSourceRecord({ companyId });
  ...
  console.log(`   ${chunks.length} chunks, ${(fullText.length / 1000).toFixed(0)}k chars`);
  ...
  const failLog = [analysisR, roadmapR, quotesR, notesR, financialsR]
    ...
    .filter(Boolean);
  if (failLog.length) failLog.forEach((m) => console.warn(`   ${m}`));
  ...
  const VALID_SUBTAGS = ['revenue-guidance', 'margin', 'capex', 'AI', 'china-risk', 'competition', 'product-launch', 'hiring', 'buyback', 'debt', 'macro', 'product'];
  const createdQuoteIds = [];

  for (const q of quotes) {
    try {
      const subTags = (q.subtags || []).filter((t) => VALID_SUBTAGS.includes(t));
      const analystNote = [
        q.analyst_note_en ? `EN: ${q.analyst_note_en}` : null,
        q.analyst_note_th ? `TH: ${q.analyst_note_th}` : null,
      ].filter(Boolean).join('\n\n');
      ...
      dot(false);
      if (process.env.DEBUG) console.error(err.message);
    }
  }
  console.log(` ${createdQuoteIds.length}/${quotes.length} quotes`);
  ...
  for (const item of roadmap) {
    try {
      const followUpText = [
        item.follow_up_en ? `EN: ${item.follow_up_en}` : null,
        item.follow_up_th ? `TH: ${item.follow_up_th}` : null,
      ].filter(Boolean).join('\n\n');
      ...
      const page = await createPage(DB.roadmap, cleanProps({
        ...
        'Date Said': n.date(DOC_DATE),
        'Follow Up Date': n.date(followUpDate),
      }));
  ...
  console.log(` ${createdRoadmapIds.length}/${roadmap.length} roadmap items`);
  ...
  for (const note of notesData) {
    try {
      ...
      const page = await createPage(DB.notes, cleanProps({
        Title: n.title(note.title_en || `${TICKER} ${DOC_TYPE} ${YEAR} ${note.note_type || 'Analysis'}`),
        Company: n.relation([companyId]),
        'Source Doc': n.relation([sourceId]),
  ...
  console.log(` ${createdNoteIds.length}/${notesData.length} notes`);

  console.log('STEP 9 — Update Company record');
  if (analysis) {
    const investThesis = [
      analysis.investment_thesis_draft ? `EN: ${analysis.investment_thesis_draft}` : null,
      analysis.investment_thesis_draft_th ? `TH: ${analysis.investment_thesis_draft_th}` : null,
    ].filter(Boolean).join('\n\n').slice(0, 2000);

    const validConviction = ['high', 'medium', 'low', 'watch'];
    const validTone = ['bullish', 'cautious', 'mixed', 'neutral'];
    ...
    console.log(`   CEO        : ${analysis.ceo}`);
  }

  if (financials) {
    console.log('STEP 10 — Update financials in data.json');
    const dataJson = loadDataJson(TICKER);
    if (!dataJson.financials) {
      dataJson.financials = { currency: 'USD', unit: 'Billion', links: [], years: [], metrics: [], notes: '' };
    }
    ...
    if (financials.guidance) {
      ...
      const guidanceText = [
        g.revenue_guidance ? `Revenue guidance: ${g.revenue_guidance}` : null,
        g.capex_guidance ? `Capex guidance: ${g.capex_guidance}` : null,
        g.other_guidance ? `Other: ${g.other_guidance}` : null,
        financials.notable_items ? `Notable: ${financials.notable_items}` : null,
      ].filter(Boolean).join(' | ');
  ...
  console.log(`Notes      : ${createdNoteIds.length}`);
